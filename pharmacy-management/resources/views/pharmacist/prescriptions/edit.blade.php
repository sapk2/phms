...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-4">Edit Prescription</h1>
    <hr class="border-b-2 border-gray-300 mb-4">
    
    <form action="{{ route('pharmacist.prescriptions.update', $prescription->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <select name="user_id" id="user_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                @foreach($pharmacists as $pharmacist)
                    <option value="{{ $pharmacist->id }}" {{ $prescription->user_id == $pharmacist->id ? 'selected' : '' }}>{{ $pharmacist->name }}</option>
                @endforeach
            </select>
            @error('user_id')
                <span class="text-red-500 text-sm">{{ $message }}</span>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="doctor_name" id="doctor_name" value="{{ old('doctor_name', $prescription->doctor_name) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            @error('doctor_name')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>

        <button type="submit" class="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600">Update Prescription</button>
