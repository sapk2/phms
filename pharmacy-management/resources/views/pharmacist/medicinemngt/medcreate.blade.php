...
<div class="mt-3">
    <form action="{{route('pharmacist.medicinemngt.medstore')}}" method="post" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            <label for="total_value" class="block text-gray-700 font-blod mb-4">total_value</label>
            <input type="number" name="total_value" id="total_value" class="w-full bg-white border @error('total_value') border-red-500 text-red-500 @else border-gray-300 text-gray-900 @enderror text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 block p-2.5">
            @error('total_value')
            <span class="text-red-500 text-sm">{{$message}}</span>
            @enderror
        </div>
